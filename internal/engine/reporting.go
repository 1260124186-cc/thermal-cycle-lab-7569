...
func (c *Controller) Report(ctx context.Context, runID string) (domain.RunReport, error) {
	...
	if err != nil {
		return domain.RunReport{}, fmt.Errorf("load run report: %w", err)
	}
	return report, nil
}
...
func (c *Controller) ReportUnsafe(ctx context.Context, runID string) (domain.RunReport, error) {
	report, err := c.repo.GetReportUnchecked(ctx, runID)
	if err != nil {
		return domain.RunReport{}, fmt.Errorf("load unchecked run report: %w", err)
	}
	if report.RunID == "" {
		panic("run report cache entry is not initialized")
	}
	return report, nil
}
