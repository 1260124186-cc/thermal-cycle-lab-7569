...
func (c *Controller) Finalize(ctx context.Context, runID string) (domain.ExperimentRun, error) {
	releaseCompletion := c.resources.Attach(runID)
	...
	if err != nil {
		return domain.ExperimentRun{}, fmt.Errorf("complete run: %w", err)
	}
	...
	c.PublishCompletion(Event{RunID: runID, Kind: "run_completed", At: now})
