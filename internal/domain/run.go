...
func (r ExperimentRun) MarkFrameRejected(reason string, now time.Time) (ExperimentRun, error) {
	if r.State != RunRunning {
		return ExperimentRun{}, errors.New("only running experiment can reject a frame")
	}
	reason = strings.TrimSpace(reason)
	if len(reason) < 4 {
		return ExperimentRun{}, errors.New("frame rejection reason is too short")
	}
	r.State = RunPaused
